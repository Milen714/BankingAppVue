  ...
  <div class="rounded-2xl border border-slate-200 bg-[#f8f8f8] p-6 shadow-sm sm:p-7">
    ...
    <form class="mt-6 space-y-5" novalidate @submit.prevent="emit('submit')">
      <div>
        ...
        <p v-if="props.shouldShowEmailError" class="mt-1.5 text-xs text-rose-600">
          {{ props.emailError }}
        </p>
      </div>
      ...
      <div>
        <div class="mb-1.5 flex items-center justify-between">
          ...
          <a href="#" class="text-sm font-medium text-amber-700 hover:text-amber-800"
            >Forgot Password?</a
          >
        </div>
        <div
        ...
        <p v-if="props.shouldShowPasswordError" class="mt-1.5 text-xs text-rose-600">
          {{ props.passwordError }}
        </p>
      </div>
      ...
      <div class="pt-1">
        <div class="relative">
          ...
          <p
            class="absolute inset-x-0 -top-2.5 mx-auto w-fit bg-[#f8f8f8] px-2 text-sm text-slate-500"
          >
            Or sign in with
          </p>
        </div>

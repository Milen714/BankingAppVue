...
const isFormValid = computed(() => {
  return !fullNameError.value && !emailError.value && !passwordError.value && !confirmPasswordError.value
})
...
<template>
  <section class="relative min-h-screen overflow-hidden bg-slate-100">
    <div class="absolute inset-0 bg-[linear-gradient(180deg,_#f8fafc_0%,_#eef4fb_42%,_#e2e8f0_100%)]"></div>
    <div class="absolute inset-x-0 top-0 h-72 bg-[radial-gradient(circle_at_top,_rgba(15,23,42,0.10),_transparent_60%)]"></div>
    ...
    <div class="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-emerald-300/80 to-transparent"></div>

    <div class="relative mx-auto flex min-h-screen max-w-6xl items-center px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
      <div class="grid w-full items-center gap-6 md:gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:gap-10">
        <div class="order-2 lg:order-1">
          <div class="mb-4 rounded-[1.75rem] border border-slate-200/80 bg-white/85 p-4 shadow-sm backdrop-blur sm:p-5 lg:hidden">
            <div class="flex items-start justify-between gap-4">
              <div>
                <div class="inline-flex items-center gap-3 rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700">
                  <span class="flex h-7 w-7 items-center justify-center rounded-full bg-slate-900 text-xs text-white">
                    B
                  </span>
                  BrightPath
                </div>
              ...
              <div class="hidden rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-right sm:block">
          ...
          <div class="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.10)] sm:rounded-[2rem]">
            ...
            <div class="mb-6 flex items-center gap-3 rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
              <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-emerald-700 shadow-sm">
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <path d="M12 12a4 4 0 100-8 4 4 0 000 8zM5 20a7 7 0 1114 0" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
              </div>
            ...
            <form class="space-y-5" novalidate @submit.prevent="handleSubmit">
              <div class="space-y-2">
                ...
                <div
                  ...
                  :class="shouldShowFullNameError ? 'border-rose-300' : 'border-slate-200 hover:border-slate-400'"
                >
                  <input
                    id="fullName"
                    v-model="fullName"
              ...
              <div class="space-y-2">
                ...
                <div
                  class="group rounded-2xl border bg-slate-50 transition duration-200 ease-out focus-within:-translate-y-0.5 focus-within:border-emerald-600 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(5,150,105,0.08)]"
                  :class="shouldShowEmailError ? 'border-rose-300' : 'border-slate-200 hover:border-slate-400'"
                >
                  <input
                    id="email"
                    v-model="email"
              ...
              <div class="grid gap-5 sm:grid-cols-2">
                <div class="space-y-2">
                  ...
                  <div
                    ...
                    :class="shouldShowPasswordError ? 'border-rose-300' : 'border-slate-200 hover:border-slate-400'"
                  >
                    <input
                      id="password"
                      v-model="password"
                ...
                <div class="space-y-2">
                  ...
                  <div
                    class="group flex rounded-2xl border bg-slate-50 transition duration-200 ease-out focus-within:-translate-y-0.5 focus-within:border-emerald-600 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(5,150,105,0.08)]"
                    :class="shouldShowConfirmPasswordError ? 'border-rose-300' : 'border-slate-200 hover:border-slate-400'"
                  >
                    <input
                      id="confirmPassword"
        ...
        <div class="order-1 hidden lg:block lg:order-2">
          <div class="max-w-lg lg:ml-auto">
            <div class="inline-flex items-center gap-3 rounded-full border border-slate-300/70 bg-white/80 px-4 py-2 text-sm font-medium text-slate-700 shadow-sm backdrop-blur">
              <span class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-white">
                B
              </span>
              BrightPath
            </div>
                ...
                <div class="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                      A simple form with clear steps
                    </p>
                  </div>
                  <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-600 text-white shadow-lg shadow-emerald-200">
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                      <path d="M12 4h7v7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" />
                      <path d="M20 4l-9 9" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" />
                      <path d="M20 13v5a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                  </div>
                </div>
                ...
                <div class="rounded-[1.75rem] border border-slate-200 bg-white/90 p-5 shadow-sm">
                  <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-2xl bg-sky-50 text-sky-700">
                      <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                        <path d="M12 21s7-4.35 7-10V6l-7-3-7 3v5c0 5.65 7 10 7 10z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" />
                      </svg>
                    </div>
                    <div>
                      <p class="text-sm font-semibold text-slate-900">
